...
dnn = Dense(2048,activation='relu')(input_audio)
...
dnn = Dense(fea_dim,activation='linear')(dnn)   ### the middle frame out

autoencoder=Model(input=input_audio,output=dnn)
...
dump_fd=cfg.model_fd+'/sednn_keras_logMag_Relu2048layer1_1outFr_7inFr_dp0.2_weights.{epoch:02d}-{val_loss:.2f}.hdf5'
