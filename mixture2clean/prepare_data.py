...
def create_mixture_csv(args):
    ...
    for speech_na in speech_names:
        ...
        if data_type == 'train':
            selected_noise_names = rs.choice(noise_names, size=args.magnification, replace=False)
        elif data_type == 'test':
...
def calculate_mixture_features(args):
    ...
    for i1 in xrange(1, len(lis)):
        ...
        out_audio_path = os.path.join(workspace, "mixed_audios", "spectrogram", data_type, "%ddb" % int(snr), "%s.wav" % out_bare_na)
        ...
        # Write out features. 
        out_feat_path = os.path.join(workspace, "features", "spectrogram", data_type, "%ddb" % int(snr), "%s.p" % out_bare_na)
