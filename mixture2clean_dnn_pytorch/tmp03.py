...
class SnrGenerator(object):
    ...
    def __next__(self):
        return self.rs.choice([-5, 0, 5], p=[1./3., 1./3., 1./3.])
...
def train(args):
    ...
    t1 = time.time()
    batch_size = 500
    snr_generator = SnrGenerator()
    data_loader = pp_data.DataLoader(hdf5_file, data_type, audio_type, stack_num, hop_frames, center_only=True, batch_size=batch_size, snr_generator=snr_generator)
    eval_tr_data_loader = pp_data.DataLoader(hdf5_file, 'train', audio_type, stack_num, hop_frames, center_only=True, batch_size=batch_size)
    eval_te_data_loader = pp_data.DataLoader(hdf5_file, 'test', audio_type, stack_num, hop_frames, center_only=True, batch_size=batch_size)
    print("Load time: %s" % (time.time() - t1))
    
    # Load scalar
    scalar_path = os.path.join(workspace, "scalars", filename, "scalar.p")
    (mean_, std_) = cPickle.load(open(scalar_path, 'rb'), encoding='latin1')
...
def inference(args):
    ...
    audio_dir = "/vol/vssp/msos/qk/workspaces/speech_enhancement/mixed_audios/spectrogram/train/0db"
    ...
    for (cnt, name) in enumerate(names):
        if cnt % n_every == 0:
            ...
            if visualize:
                ...
                axs[1].matshow(np.log(np.abs(pred_mag_sp)).T, origin='lower', aspect='auto', cmap='jet')
                plt.show()
